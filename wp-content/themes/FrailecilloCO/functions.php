...
if ( ! function_exists( 'lobo_pagination' ) ) {
	...
	function lobo_pagination( $query = null ) {  
		...
		if( $pages > 1 ) {
			...
			if ( $page + 1 <= $pages ) {
				$html .= '<div class="prev-post"><a href="' . get_pagenum_link( $page + 1 ) . '">' . '<div class="pagination-inner">' . __( 'Older Posts' ,'lobo' ) . '</div></a></div>';
			}

			if ( $page - 1 >= 1 ) {
				$html .= '<div class="next-post"><a href="' . get_pagenum_link( $page - 1 ) . '"><div class="pagination-inner">' . __( 'Newer Posts' ,'lobo' ) . '</div></a></div>';
			}
...
if ( ! function_exists( 'lobo_setup' ) ) {
	...
	function lobo_setup() {
		...
		if ( ot_get_option( 'krown_meta_enable', 'enabled' ) == 'enabled' ) {
...
if ( ! function_exists( 'krown_social_meta' ) ) {
	...
	function krown_social_meta() {
		...
		if ( is_singular() ) {
			...
			echo '<meta property="og:description" content="' . wp_strip_all_tags( lobo_excerpt( 'lobo_excerptlength_post' ) ) . '" />';
...
function lobo_enqueue_scripts() {
	...
	if ( isset( $post ) && ( is_singular( 'product' ) || is_page_template( 'template-portfolio.php' ) ) ) {
	...
	wp_localize_script(
		...
		array(
			...
			'wooCommerce23' => lobo_is_wc_version_gte_2_3()
		)
	);
	...
	function krown_hero( $post ) {
		...
		switch ( get_post_meta( $post_id, 'krown_page_hero', true ) ) {
			...
			case 'video':
				...
				$hero_el .= '<figure class="hero-video hero-item" data-fallback="' . get_post_meta( $post_id, 'krown_hero_img', true ) . '">';
        ...
        if ( ! empty ( $kmb_decoded ) ) {
        	...
        	foreach ( $kmb_decoded as $module ) {
        		...
        		if ( isset( $module->type ) ) {
	        		...
	        		if ( $module->type != 'nav' && $module->type != 'slider' && $module->type != 'latest' && $module->type != 'related' && $module->type != 'separator' ) {
	                    ...
	                    if ( $module->type == 'map' ) {
			            	...
			            	$overlay = 'yes';
			            	if ( isset( $module->overlay ) ) {
			            		$overlay = $module->overlay;
			            		$module->overlay = '';
			            	}

		            		$content .= '</div><a class="galleria-run overlay-' . $overlay . '" href="#"><div><span class="text"><span class="icon"></span>' . $label  . '</span></div></a><div class="hidden galleria-btn-labels"><span class="gal-next">' . __( 'next', 'lobo' ) . '</span><span class="gal-prev">' . __( 'prev', 'lobo' ) . '</span><span class="gal-close">' . __( 'close', 'lobo' ) . '</span></div>';
	            		...
	            		$html .= '<div id="module-' . $post->ID . $i++ . '" class="module default-module no-resize" data-size="' . $module->width . '-' . $module->height . '"><div class="royalSlider rsDefault' . ( isset( $module->transition ) ? ' will-' . $module->transition : '' ) . '">';
	            	...
	            	} else if ( $module->type == 'related' ) {

	            		// Related portfolio, special element
...
add_action( 'admin_notices', 'lobo_update_notice' );

function lobo_update_notice() {

	if ( get_option( 'lobo_version' ) != '2.3.1' ) {

        echo '<div class="updated" style="position: relative;">
        	<h4>You have just updated to version 2.3.1 - <a style="text-decoration" href="' . admin_url( 'themes.php?page=ot-theme-options&lobo_update_done_do=1#section_log' ) . '">Read the CHANGELOG</a></h4>';

        printf(__('<em style="position: absolute; top: 18px; right: 20px;"><a href="%1$s">Dismiss</a></em>'), '?lobo_update_done_do=1');
...
add_action( 'admin_init', 'lobo_update_done_do' );

function lobo_update_done_do() {
	global $current_user;
    $user_id = $current_user->ID;
    if ( isset( $_GET['lobo_update_done_do'] ) && '1' == $_GET['lobo_update_done_do'] ) {
        update_option( 'lobo_version', '2.3.1' );
	}
}

// Check WooCommerce version

function lobo_get_wc_version() {
	return defined( 'WC_VERSION' ) && WC_VERSION ? WC_VERSION : null;
}
function lobo_is_wc_version_gte_2_3() {
	return lobo_get_wc_version() && version_compare( lobo_get_wc_version(), '2.3', '>=' );
}
